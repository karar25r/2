...
table.add_column("The Tool By : abod «𓆩ᶠᴮᴵ𓆪» ™",style='green')
table.add_row("My User : @B_6_Q",style='green')
table.add_row("My Channel : https://t.me/ModcaTheLost",style='Cyan')
table.add_row("My Channel 2 : https://t.me/ModcaTheLost",style='yellow')
console = Console()
console.print(table)
abod = Console()
tok = input(O+' -Enter Your Tokne : ')
print('\n')
iD = input(O+' -Enter Your ID : ')
print('  '*15)
with abod.status(F+'please wait !  ',spinner_style='point'):
        time.sleep(2.5);requests.post(f"https://api.telegram.org/bot{tok}/sendvideo?chat_id={iD}&video=https://t.me/yyyyyy3w/20&caption=")
print('             '*10)
def abodgmail(email):
...
'Cookie': 'missing' }
        data = {'uuid':uid,  'password':'@abodFurry',
              'username':email,
              'device_id':uid,
              'from_reg':'false',
              '_csrftoken':'missing',
...
⋘───━𓆩abod «𓆩ᶠᴮᴵ𓆪» ™𓆪‏━───⋙
𝘽𝙔 ➥ """)
                                        requests.post(f"https://api.telegram.org/bot{tok}/sendMessage?chat_id={iD}&text="+str(abodhunt))
                                except:
                                        abodhunt = (f"""
𝙃𝙀𝙇𝙇𝙊 𝘽𝙍𝙊 𝙉𝙀𝙒 𝘼𝘾𝘾 𝙄𝙂  ⚡
⋘───━𓆩abod «𓆩ᶠᴮᴵ𓆪» ™𓆪‏━───⋙
...
def abodlist():
 while True:
  try:
        ...
        abod_E=0
abod_prox = threading.Thread(target=abodlist,args=())
abod_prox.start()
proxy_list = []
for i in range(2):
                      t = threading.Thread(target=abodlist)
                      t.start()
                      proxy_list.append(t)
                      time.sleep(0.01)
